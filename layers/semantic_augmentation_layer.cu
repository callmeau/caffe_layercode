...
__global__ void WarpData(const int nthreads, const int num, const int height, const int width, const Dtype* src_data, const int src_count,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    // Step 2: Difference between the new and old positions gives the semantic
    int srcIdxOff = width*(height*(2*n+0) + 
                     (int)(ypos1+(Dtype)0.5)) + 
                     (int)(xpos1+(Dtype)0.5);
    dest_data[dest_width*(dest_height*(2*n+0) + (int)y) + (int)x] = src_data[min(srcIdxOff, src_count)];
    dest_data[dest_width*(dest_height*(2*n+1) + (int)y) + (int)x] = 0;
  }
}


template <typename Dtype>
void SemanticAugmentationLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
  //"Semantic augmentation layer takes three input blobs: SemanticField, Img1TransfParams, Img2TransfParams";
...
INSTANTIATE_LAYER_GPU_FUNCS(SemanticAugmentationLayer);
