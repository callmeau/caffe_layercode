...
#include "caffe/layers/semantic_augmentation_layer.hpp"
...
void SemanticAugmentationLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top)
{
    ...
    LOG(WARNING) << "SemanticAugmentationLayer only runs Reshape only on setup";
}

template <typename Dtype>
void SemanticAugmentationLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top)
{
  CHECK_EQ(bottom.size(), 3) << "Semantic augmentation layer takes three input blobs: SemanticField, Img1TransfParams, Img2TransfParams";
  CHECK_EQ(top.size(), 1) << "Semantic augmentation layer outputs one output blob: Augmented Semantic";
  ...
  CHECK_EQ(channels, 2) << "Semantic data must have two channels";
...
template <typename Dtype>
void SemanticAugmentationLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top)
{  
...
#ifdef CPU_ONLY
STUB_GPU(SemanticAugmentationLayer);
#endif

INSTANTIATE_CLASS(SemanticAugmentationLayer);
REGISTER_LAYER_CLASS(SemanticAugmentation);
