...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            top[0].data[itt*2, ...] = self.transformer.preprocess('data',im_L)
            top[0].data[itt*2+1, ...] = self.transformer.preprocess('data',im_R)
            top[1].data[itt,:3,...] = self.transformer1.preprocess('data',im_L)/255
            top[1].data[itt,3:,...] = self.transformer1.preprocess('data',im_R)/255
            for ic,scale in enumerate( self.multi_scale ):
                sl = 2**scale
