...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        #self.need = [8,17,19,20,24,27,28,29,30,31,32,12,13,22,25,33]
        #self.need = [17, 19, 25, 32, 33]
        self.need = [8,12,17,19,20,24,27,28,31,32,13,22,25,33]
        self.to_need = 0
        self.to_aug = 0
    ...
    def reshape(self, bottom, top):
        ...
        labelsList = []
        for itt in range(self.batch_size):
            im, label, flags = self.batch_loader.load_next_image()
            imgsList.append(im)
            labelsList.append(label)
        pro = .8
        ot = 2
        if random.random() < pro:
            ...
            if  random.random() < pro:
                ...
                for itt in range(self.batch_size):
                    im = imgsList[itt]
                    label = labelsList[itt]
                    if np.abs(rotate_angle) < .5 :
                        im,label = rotate(im,label, im.size[1] - 32, 2*(im.size[1]-32), rotate_angle)
                    else:
                        im,label = rotate(im,label, im.size[1] - 64, 2*(im.size[1]-64),rotate_angle)
                    imgsList[itt] = im;
                    labelsList[itt] = label;
            for itt in range(self.batch_size):
				if random.random() < pro :
        ...
        for itt in range(self.batch_size):
            ...
            label = assign_trainIds(self.id2trainId,label, self.need,self.to_need)
            #scmi.imsave('kitti_test.png',label)
            #scmi.imsave('kitti_img.png',im)
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        im = Image.open(osp.join(lp.kitti_seg,left_name))
        label = Image.open( osp.join(lp.kitti_seg,label_name))
        #y1,x1 = crop_image((im.size[1],im.size[0]),self.im_shape,tag = self.tag)
        im = im.resize((self.im_shape[1],self.im_shape[0]))
        label = label.resize((self.im_shape[1],self.im_shape[0]),Image.NEAREST)
        if random.random() < progate :
            # color saturation change
            color_factor = np.random.uniform(0.6,1.4)
            ...
            # contrast change
            contrast_factor = np.random.uniform(0.6,1.4)    # 阈值需要重新确定？
            im = ImageEnhance.Contrast(im).enhance(contrast_factor)

        
        self._cur = self._cur + 1 
        return im,label,flags

def assign_trainIds(id2trainId, label, need=None, to_need = None):
