...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.params = params
        self.size_list=[384,320,256,192]
    ...
    def reshape(self, bottom, top):
        ...
        pro = .5
        ot = 2
        flag = 448 if im_L.size[1] >= 384 else 256
        if random.random() < pro:
            if random.random() < pro:
                scale = 1.5*random.random() + 0.6
            ...
            if  random.random() < pro-0.2  and im_L.size[1] >= flag + 64:
                rot = random.uniform(-5, 5)
                im_L,label = rotate(im_L,label,scale-64,2*(scale-64),rot)
                im_R,disp = rotate(im_R,disp,scale-64,2*(scale-64),rot)
            if random.random() < pro-0.2 :
        ...
        disp = (np.asarray(disp,dtype = np.float32)-1)/255
        y1,x1 = crop_image(im_L.shape,im_shape,tag = params['tag'])
        self.im_L = im_L[ y1:y1+im_shape[0],x1:x1+im_shape[1],:]
        self.im_R = im_R[ y1:y1+im_shape[0],x1:x1+im_shape[1],:]
        ...
        transformer1 = Transformer({'data':(self.batch_size,3,im_shape[0], im_shape[1])})
        ...
        # once. Else, we'd have to do it in the reshape call.
        top[0].reshape(
            self.batch_size, 6, im_shape[0], im_shape[1])
        top[1].reshape(
            self.batch_size*2, 3,im_shape[0], im_shape[1])
        self.seg_scale = params['seg_scale']
        for ic,scale in enumerate( self.seg_scale ):
            sl = 2**scale
            top[ic+2].reshape(
                self.batch_size, 1, im_shape[0]/sl, im_shape[1]/sl)
        self.disp_scale = params['disp_scale']
        for ic,scale in enumerate( self.disp_scale ):
            sl = 2**scale
            ic += len(self.seg_scale)
            top[ic+2].reshape(
                self.batch_size, 1, im_shape[0]/sl, im_shape[1]/sl)
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im_L,im_R,seg,disp = self.im_L,self.im_R,self.label,self.disp
            # Add directly to the caffe data layer
            top[0].data[itt,:3,...] = 1.0*self.transformer1.preprocess('data',im_L)/255
            top[0].data[itt,3:,...] = 1.0*self.transformer1.preprocess('data',im_R)/255
            top[1].data[itt*2,...] = self.transformer.preprocess('data',im_L)
            top[1].data[itt*2+1,...] = self.transformer.preprocess('data',im_R)
            for ic,scale in enumerate( self.seg_scale ):
                sl = 2**scale
                top[ic+2].data[itt, ...] = seg[np.newaxis,::sl,::sl]
            for ic,scale in enumerate( self.disp_scale ):
                sl = 2**scale
                ic += len(self.seg_scale)
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        if index.find('data_scene_flow') == -1:
            ...
            left_name,right_name,disp_name,seg_name=index.split()
            img_L = Image.open(osp.join(lp.kitti_disp,left_name))
            img_R = Image.open(osp.join(lp.kitti_disp,right_name))
            size = (640,192)
            img_scale1 = img_L.resize(size, Image.BILINEAR)
            img_scale2 = img_R.resize(size, Image.BILINEAR)
            disp = Image.open(osp.join(lp.kitti_disp,disp_name))
            seg = Image.open(osp.join(lp.kitti_seg,seg_name))
        if random.random() < 0.3:
            # color saturation change
            color_factor = np.random.uniform(0.8,1.2)
            img_scale1 = ImageEnhance.Color(img_scale1).enhance(color_factor)
            img_scale2 = ImageEnhance.Color(img_scale2).enhance(color_factor)

            # brightness change
            brightness_factor = np.random.uniform(0.8,1.2)
            img_scale1 = ImageEnhance.Brightness(img_scale1).enhance(brightness_factor)
            img_scale2 = ImageEnhance.Brightness(img_scale2).enhance(brightness_factor)
            # contrast change
            contrast_factor = np.random.uniform(0.8,1.2)    # 阈值需要重新确定？
            img_scale1 = ImageEnhance.Contrast(img_scale1).enhance(contrast_factor)
            img_scale2 = ImageEnhance.Contrast(img_scale2).enhance(contrast_factor)
        #temp = static_pixes(label,alpha = self.alpha,channel=self.label_num)
        self._cur = self._cur + 1 
        return img_scale1,img_scale2,seg,disp
...
def rotate(img, label, new_height, new_width,rotate_angle):
