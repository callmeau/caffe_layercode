...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def reshape(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            if flags and not self.to_train:
                label = assign_trainIds(self.id2trainId,label, self.need,self.to_need)
            else:
                res = 19*np.ones_like( label, dtype ='int8')
                for iv in self.need:
                    lv = self.id2trainId[iv]
                    res[label == lv] = lv
                label = res
            #scmi.imsave('forTest/'+self.batch_loader.img_name, label)
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        flags = True
        if label_name.find('result') != -1:
            flags = False
        self.img_name = left_name.split('/')[-1]
