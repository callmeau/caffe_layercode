...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        indexlist = [line.rstrip('\n') for line in open(osp.join(lp.aug_info_root,list_file))]
        ...
        for ic,scale in enumerate( self.multi_scale ):
            sl = 2**scale
            top[ic+1].reshape(
                self.batch_size, 2, params['im_shape'][0]/sl, params['im_shape'][1]/sl)
        '''
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im_L,im_R,optical = self.batch_loader.load_next_image()
            optical = optical.transpose(2, 0, 1)
            # Add directly to the caffe data layer
            top[0].data[itt, :3,...] = self.transformer1.preprocess('data',im_L)/255
            top[0].data[itt, 3:,...] = self.transformer1.preprocess('data',im_R)/255
            for ic,scale in enumerate( self.multi_scale ):
                sl = 2**scale
                top[ic+1].data[itt, 0, ...] = optical[1,::sl,::sl]
                top[ic+1].data[itt, 1, ...] = optical[0,::sl,::sl]
            '''
            if self.to_temp:
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        left_name,right_name,optical_name=index.split()
        # load a image and its label
        '''
        img_L = Image.open(osp.join(lp.aug_image_root,left_name))
        img_R = Image.open(osp.join(lp.aug_image_root,right_name))
        disp = Image.open(osp.join(lp.aug_image_root,disp_name))
        '''
        img_L = Image.open(left_name)
        img_R = Image.open(right_name)
        optical = Image.open(optical_name)
        ...
        optical_scale = optical.resize( size, Image.BILINEAR)
        ...
        optical_scale = (np.asarray(optical_scale,dtype = np.float32))
        ...
        self._cur += 1
        return img_scale1,img_scale2,optical_scale
