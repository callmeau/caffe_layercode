...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            for ic,scale in enumerate( self.multi_scale ):
                sl = 2**scale
                top[ic+2].data[itt, ...] = label[np.newaxis,::sl,::sl]
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        left_name,right_name,label_name,coord,_=index.split()
        #image_file_name 
        ...
        im_L = im_L[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1],:]


        im_R = np.asarray(Image.open(right_name))
        im_R = im_R[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1],:]
        ...
        #self._cur = (self._cur+1) % 10
        self._cur += 1
        return self.transformer.preprocess('data',im_L),self.transformer.preprocess('data',im_R),label
