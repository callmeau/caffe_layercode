...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.params = params
        self.tag = params['tag']
        self.size_list=[640,576,512,480,384,256]
    ...
    def reshape(self, bottom, top):
        ...
        im,label = self.batch_loader.load_next_image()
        pro = .3
        #ot = random.choice([1,2])
        ot = 2
        flag = 1 if im.size[1] > 512 else 0
        if random.random() < pro:
            if random.random() < pro:
                scale = 0.4*random.random() + 0.8
                scale = int(scale * im.size[1])
                scale = scale // 32 * 32
                im = im.resize((scale*2,scale))
                label = label.resize( (scale*2,scale),Image.NEAREST)
            if  random.random() < pro-0.2  and im.size[1] >= 480 and 0:
        ...
        self.im = im[ y1:y1+im_shape[0],x1:x1+im_shape[1],:]
        label = label[ y1:y1+im_shape[0],x1:x1+im_shape[1]]
        label = assign_trainIds(self.id2trainId,label,self.need)
        self.label = label
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im = self.transformer.preprocess('data',self.im)
            label = self.label
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        if len(items) <= 2:
            ...
            im = Image.open( lp.image_root+left_name ).resize((1024,512))
            label = Image.open( lp.image_root+label_name).resize((1024,512),Image.NEAREST)
        else:
            left_name,label_name,corrd = items
            start,ends = corrd.split(',')
            s = int(start)
            e = int(ends)
            im = Image.open(lp.image_root+left_name).crop((s,e,s+self.im_shape[1],e+self.im_shape[0]))
            label = Image.open(lp.image_root+label_name).crop((s,e,s+self.im_shape[1],e+self.im_shape[0]))
...
def assign_trainIds(id2trainId, label,need):
    for k, v in id2trainId.iteritems():
        if v!= 255 and v in need:
            label[label == k] = v
        else:
            label[label == k] = 19
    return label
