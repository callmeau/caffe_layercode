...
sys.path.append('/home/zhanwj/Desktop/scenflow/flownet2/python')
import scipy.misc
import skimage.io
import caffe
...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        indexlist = [line.rstrip('\n') for line in open(osp.join(lp.fuse_info_root,list_file))]
        ...
        self.batch_loader = BatchLoader(params,indexlist,None,None)
    ...
    def reshape(self, bottom, top):
        ...
        (rows and columns)
        """
        self.im_L, self.im_R, self.disp = self.batch_loader.load_next_image()
        #print self.im_L.shape
        transformer1 = Transformer({'data':(self.batch_size,3, self.im_L.shape[0], self.im_L.shape[1])})
        transformer1.set_transpose('data',(2,0,1))
        transformer1.set_channel_swap('data',(2,1,0))
        self.transformer1 = transformer1
        top[0].reshape(
            self.batch_size, 6, self.im_L.shape[0],  self.im_L.shape[1])
        #print top[0].data[0].shape
        for ic,scale in enumerate( self.multi_scale ):
            sl = 2**scale
            top[ic+1].reshape(
                self.batch_size, 1, self.im_L.shape[0]/sl, self.im_L.shape[1]/sl)      
        #print top[1].data[0].shape
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            top[0].data[itt, :3,...] = 1.0*self.transformer1.preprocess('data',self.im_L)/255
            top[0].data[itt, 3:,...] = 1.0*self.transformer1.preprocess('data',self.im_R)/255
            for ic,scale in enumerate( self.multi_scale ):
                sl = 2**scale
                top[ic+1].data[itt, ...] = self.disp[np.newaxis,::sl,::sl]
            '''
            if self.to_temp:
...
class BatchLoader(object):
    ...
    def __init__(self, params, indexlist,transformer,result):
        ...
        #统一压缩为360,480
        self.im_shape_Cityscapes = params['im_shape_City']
        self.im_shape_KITTI = params['im_shape_KITTI']
        #要修正文件存放位置,存放格式:/home/zhanwj/data/1.jpg 1000100100
    ...
    def load_next_image(self):
        ...
        left_name,right_name,disp_name=index.split()
        img_L, img_R, disp = None, None, None
        # load a image and its label
        if 'leftImg8bit_trainvaltest' in left_name:	
            img_L = Image.open(osp.join(lp.city_image_root,left_name))
            img_R = Image.open(osp.join(lp.city_image_root,right_name))
            disp = Image.open(osp.join(lp.city_image_root,disp_name))
        else:
            img_L = Image.open(osp.join(lp.kitti_image_root,left_name))
            img_R = Image.open(osp.join(lp.kitti_image_root,right_name))
            disp = Image.open(osp.join(lp.kitti_image_root,disp_name))			
        #seg = Image.open(osp.join(lp.image_root,seg_name))
        if img_L.size[0] == 2048:
            size = self.im_shape_Cityscapes[::-1]
        else:
            size = self.im_shape_KITTI[::-1]
        ...
        img_scale1 = np.asarray( img_scale1 ) 
        img_scale2 = np.asarray( img_scale2 ) 
        ...
        self._cur += 1
        return img_scale1,img_scale2,disp_scale
...
def check_params(params):
    ...
    required = ['batch_size',  'im_shape_KITTI', 'im_shape_City']
    for r in required:
...
def print_info(name, params):
    ...
    print "{} initialized for split: {}, with bs: {}, im_shape: {}.".format(
        ...
        params['split'],
        params['batch_size'],
        params['im_shape_KITTI'],
		params['im_shape_City'])
...
def rotate(img, label, new_height, new_width):
    ...
    rotate_angle = random.uniform(0, 0)
