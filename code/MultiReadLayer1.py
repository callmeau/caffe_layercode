...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        transformer1 = Transformer({'data':(self.batch_size,3,params['im_shape'][0], params['im_shape'][1])})
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im_L,im_R,seg,disp = self.batch_loader.load_next_image()
            # Add directly to the caffe data layer
            top[0].data[itt,...] = 1.0*self.transformer1.preprocess('data',im_L)/255
            top[1].data[itt,...] = 1.0*self.transformer1.preprocess('data',im_R)/255
            top[2].data[itt*2,...] = self.transformer.preprocess('data',im_L)
            top[2].data[itt*2+1,...] = self.transformer.preprocess('data',im_R)
            for ic,scale in enumerate( self.seg_scale ):
                sl = 2**scale
                top[ic+3].data[itt, ...] = seg[np.newaxis,::sl,::sl]
            for ic,scale in enumerate( self.disp_scale ):
                sl = 2**scale
                ic += len(self.seg_scale)
                top[ic+3].data[itt, ...] = disp[np.newaxis,::sl,::sl]
            '''
            if self.to_temp:
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        seg_scale = seg.resize(size, Image.NEAREST)
        ...
        self._cur += 1
        return img_scale1,img_scale2,seg_scale,disp_scale
