...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        # Load an image
        item = self.indexlist[self._cur].replace('dispflownet-release/models','flownet2').split()  # Get the image index
        if len(item) == 2:
            index,label_name = item
        else:
            index,label_name,_ = item
        #split image_name and image_lablel
        left_name=index
        #image_file_name 
        #im_L = np.asarray(Image.open(lp.image_root+left_name))
        im = np.asarray(Image.open(left_name).resize(self.im_shape[::-1]))
        #im_L = scipy.misc.imresize(im, self.im_shape)  # resize
        #load image to label
        label = np.asarray(Image.open(label_name).resize(self.im_shape[::-1],Image.NEAREST)).astype(np.int8)
        self._cur += 1
