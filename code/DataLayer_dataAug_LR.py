...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        indexlist = [line.rstrip('\n') for line in open(osp.join(lp.disp_info_root,list_file))]
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im_L, im_R, label = self.batch_loader.load_next_image()

            # Add directly to the caffe data layer
            top[0].data[itt, :3, ...] = 1.0 * im_L / 255
            top[0].data[itt, 3:, ...] = 1.0 * im_R / 255
            for ic,scale in enumerate( self.multi_scale ):
                sl = 2**scale
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        left_name, right_name, label_name = index.split()
        # load a image and its label
        img_L = Image.open(lp.image_root+left_name)
        img_R = Image.open(lp.image_root+right_name)
        label = Image.open(lp.image_root+label_name)
        if random.random() <= 1.0:
            img_L, img_R, label = self.data_aug(img_L, img_R, label)
            if img_L.shape[0] > self.im_shape[0]:
                y1,x1 = crop_image(img_L.shape[:2],self.im_shape,tag = self.tag)
                img_L = img_L[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1],:]
                img_R = img_R[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1],:]
                label = label[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1]]
		label = (np.asarray(label, dtype=np.float32)-1) / 256
        # else:   
        ...
        self._cur += 1
        return self.transformer.preprocess('data',img_L), self.transformer.preprocess('data',img_R), label

    def data_aug(self, img_L, img_R, label):
        # Data augmentation of the loaded image
        # Color distort
        cut = .3
        prob_distort = cut
        if random.random() <= prob_distort:
            img_L, img_R = distort(img_L, img_R)

        # Rotate and crop to expected shape(fixed 640*1280)
        prob_rotate = cut
        if random.random() <= prob_rotate:
            img_L, img_R, label = rotate(img_L, img_R, label, self.im_shape[0], self.im_shape[1])
            #img = np.asarray(img).astype(np.uint8)
            #label = np.asarray(label).astype(np.uint8) 
        else:
            # Rescale and crop to 640*1280
            prob_rescale = cut
            if random.random() <= prob_rescale:
                img_L, img_R, label=rescale(img_L, img_R, label, self.im_shape[0], self.im_shape[1], True)
...
def distort(img_L, img_R):
    # color saturation change
    color_factor = 1.5*random.random()+0.5
    img_L = ImageEnhance.Color(img_L).enhance(color_factor)
    img_R = ImageEnhance.Color(img_R).enhance(color_factor)

    # brightness change
    brightness_factor = 0.8*random.random() + 0.7
    img_L = ImageEnhance.Brightness(img_L).enhance(brightness_factor)
    img_R = ImageEnhance.Brightness(img_R).enhance(brightness_factor)

    # contrast change
    contrast_factor = random.uniform(-0.8,0.4)
    img_L = ImageEnhance.Contrast(img_L).enhance(contrast_factor)
    img_R = ImageEnhance.Contrast(img_R).enhance(contrast_factor)

    return img_L, img_R

def gaussian_noise(img):
    sigma = 1.0*random.randint(0, 4) / 100
    var = sigma**2
...
def rotate(img_L, img_R, label, new_height, new_width):
    height, width = img_L.size[1], img_R.size[0]
    sh = math.floor((height - new_height) / 2)
    sw = math.floor((width - new_width) / 2)
    rotate_angle = random.uniform(-18.9, 18.9)
    img_L = img_L.rotate(rotate_angle)
    img_R = img_R.rotate(rotate_angle)
    label = label.rotate(rotate_angle)
    img_L = img_L.crop((sw, sh, sw+new_width, sh+new_height))
    img_R = img_R.crop((sw, sh, sw+new_width, sh+new_height))
    label = label.crop((sw, sh, sw+new_width, sh+new_height))
    return img_L, img_R, label

def rescale(img_L, img_R, label, new_height, new_width, randoms=False):
    scale = 1.0*random.randint(9, 21) / 10
    height, width = img_L.size[1], img_R.size[0]
    scale_height = int(math.floor(height * scale))
    scale_width = int(math.floor(width * scale))
    img_L_scale = img_L.resize((scale_width, scale_height), Image.BILINEAR)
    img_R_scale = img_R.resize((scale_width, scale_height), Image.BILINEAR)
    label_scale = label.resize((scale_width, scale_height), Image.BILINEAR)
    ...
    img_L = img_L_scale.crop((sw, sh, sw+new_width, sh+new_height))
    img_R = img_R_scale.crop((sw, sh, sw+new_width, sh+new_height))
