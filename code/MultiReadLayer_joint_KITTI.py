...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        flag =True
        progate = .3
        if index.find('-1,-1') != -1 or len(items) == 4:
            if len(items) == 4:
                left_name, right_name, label_name, disp_name = items
            else:
                left_name, right_name, label_name, disp_name, _ = items
            ...
            left_name, right_name, label_name, disp_name, corrd = items
            start,ends = corrd.split(',')
            s = int(start)
            e = int(ends)
            im_L = Image.open(left_name).crop((s,e,s+self.im_shape[1],e+self.im_shape[0]))
            im_R = Image.open(right_name).crop((s,e,s+self.im_shape[1],e+self.im_shape[0]))
            label = Image.open(label_name).crop((s,e,s+self.im_shape[1],e+self.im_shape[0]))
            disp = Image.open(disp_name).crop((s,e,s+self.im_shape[1],e+self.im_shape[0]))
