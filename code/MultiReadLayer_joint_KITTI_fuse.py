...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.size_list=[352, 320, 256,224, 192, 160, 128, 96, 64]
    ...
    def reshape(self, bottom, top):
        ...
        pro = .7
        ot = 3.3
        if random.random() < pro and self.tag == 'Train':
            if random.random() < -1:
            ...
            if  random.random() < pro:
                for itt in range(self.batch_size):
                    ...
                    label = labelsList[itt]
                    disp = dispList[itt]
                    im_L, im_R, label, disp = rotate(im_L, im_R, label, disp, im_L.size[1] - 32, (ot*(im_L.size[1]-32)))
                    imgsList[itt] = im_L
                    imgsList_R[itt] = im_R
                    labelsList[itt] = label
                    dispList[itt] = disp
        ...
        for itt in range(self.batch_size):
            ...
            disp = (np.asarray(dispList[itt], dtype=np.float64))
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        im_L = Image.open(left_name)
        im_R = Image.open(right_name)
        #im_L = Image.open(lp.kitti_image_root+left_name)
        #im_R = Image.open(lp.kitti_image_root+right_name)
        if label_name == 'NonSemantic' or label_name == '_':
            label = Image.fromarray(np.zeros((im_L.size[1], im_L.size[0])))
        else:    
            label = Image.open(lp.kitti_semantics_root+label_name)
        #disp = Image.open(lp.kitti_image_root+disp_name)
        if disp_name == '_':
            disp = Image.fromarray(np.zeros((im_L.size[1], im_L.size[0])))
        elif '.txt' in disp_name:
            disp = np.loadtxt(lp.kitti_image_root+disp_name, np.int32)
            #print np.max(disp)
            disp = Image.fromarray(disp)
        else:
            disp = Image.open(lp.kitti_image_root+disp_name)
            if 'sps' not in disp_name:
                disp = np.array(disp, np.float64)/256
                disp = Image.fromarray(disp)
