...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        if random.random() < progate:
            ...
            if random.random() < progate and flag:
                scale = 0.5*random.random() + 0.8
                sw,sh =int(scale*im.size[0]),int(scale*im.size[1])
                im = im.resize((sw,sh))
                label = label.resize((sw,sh),Image.NEAREST)
            if  random.random() < progate :
                im,label = rotate(im,label,self.im_shape[0],self.im_shape[1])
            if random.random() < progate :
                im = im.transpose(Image.FLIP_LEFT_RIGHT)
                label = label.transpose(Image.FLIP_LEFT_RIGHT)
        im = np.asarray(im)
        label = np.asarray(label).astype(np.int8)
        y1,x1 = crop_image(im.shape[:2],self.im_shape,tag = self.tag)
        im = im[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1],:]
