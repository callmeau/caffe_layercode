...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        #self.mean = np.array(params['mean'])[::-1]
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            im,im_R = self.batch_loader.load_next_image()

            # Add directly to the caffe data layer
            top[0].data[itt,:3 ,...] = im
            top[0].data[itt,3:, ...] = im_R
            top[1].data[itt*2, ...] = im
            top[1].data[itt*2+1, ...] = im_R
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        # Load an image
        index,label_name = self.indexlist[self._cur].split()  # Get the image index
        #split image_name and image_lablel
        left_name=index
        #image_file_name 
        #im_L = np.asarray(Image.open(lp.image_root+left_name))
        im = np.asarray(Image.open(left_name))
        left_im = scmi.imresize(im,self.im_shape)
        #im_L = scipy.misc.imresize(im, self.im_shape)  # resize
        #load image to label
        label = np.asarray(Image.open(label_name))
        right_im = scmi.imresize(label,self.im_shape)
        
        self._cur += 1
        return self.transformer.preprocess('data',left_im),self.transformer.preprocess('data',right_im)
