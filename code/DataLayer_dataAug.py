...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        indexlist = [line.rstrip('\n') for line in open(osp.join(lp.op_root,list_file))]
...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        left_name, label_name = index.split()
        # load a image and its label
        img = Image.open(left_name)
        label = Image.open(label_name)
        if random.random() <= 2:
            #img,label = self.data_aug(img,label)
            img = np.array(img,np.float64)/255
            ...
            if img.shape[0] > self.im_shape[0]:
                ...
                label = label[ y1:y1+self.im_shape[0],x1:x1+self.im_shape[1]]    # labels in SYN has three channels
        else:   
        ...
        self._cur += 1
        return self.transformer.preprocess('data',img),label
    def data_aug(self,img,label):
        ...
        if random.random() <= prob_distort:
            img = distort(img)
        # Add gaussian noise
        #prob_gaussianNoise = cut
        #if random.random() <= prob_gaussianNoise:
