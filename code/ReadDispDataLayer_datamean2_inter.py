...
# imports
import sys
sys.path.append('/home/zhanwj/Desktop/scenflow/flownet2/DispNet/KITTI_disp/code')
...
class PascalMultilabelDataLayerSync(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        indexlist = [line.rstrip('\n') for line in open(osp.join(lp.label_root_KITTI,list_file))]
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            for ic,scale in enumerate( self.multi_scale ):
                sl = 2**scale
                top[ic+1].data[itt, ...] = disp[np.newaxis,::sl,::sl]
            '''
            if self.to_temp:
...
class BatchLoader(object):
    ...
    def __init__(self, params, indexlist,transformer,result):
        ...
        self.pascal_root = lp.label_root_KITTI
        #统一压缩为360,480
    ...
    def load_next_image(self):
        ...
        img_L = Image.open(osp.join(lp.KITTI_disp_root,left_name))
        img_R = Image.open(osp.join(lp.KITTI_disp_root,right_name))
        #disp = Image.open(osp.join(lp.KITTI_disp_root,disp_name))
        disp = np.loadtxt(osp.join(lp.KITTI_disp_root,disp_name))
        ...
        disp_scale = (np.array(disp_scale, np.float64)-1)/256
